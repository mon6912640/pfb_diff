...
def before_info():
    if not state.before_file:
        with ui.column().classes("w-full items-center justify-center py-6"):
            ...
            ui.button("📂 浏览...", on_click=browse_before).props("flat dense").classes("text-blue-400 text-xs mt-2")
        return
        ...
        with ui.row().classes("w-full items-center justify-between"):
            with ui.column().classes("gap-0"):
                ...
                if state.before_path:
                    ui.label(state.before_path).classes("text-gray-600 text-xs mt-1").style("max-width: 280px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;")
            with ui.column().classes("gap-1 items-end"):
                ui.button("🗑 移除", on_click=remove_before).props("flat dense").classes("text-gray-500 hover:text-red-400 text-xs")
                ui.button("📂 浏览...", on_click=browse_before).props("flat dense").classes("text-blue-400 text-xs")
...
def after_info():
    if not state.after_file:
        with ui.column().classes("w-full items-center justify-center py-6"):
            ...
            ui.label("或点击选择文件").classes("text-gray-600 text-xs mt-1")
            ui.button("📂 浏览...", on_click=browse_after).props("flat dense").classes("text-blue-400 text-xs mt-2")
        return
        ...
        with ui.row().classes("w-full items-center justify-between"):
            with ui.column().classes("gap-0"):
                ...
                if info["ok"]:
                    ...
                    ui.label(f"⚠️ 解析失败: {info['error']}").classes("text-red-400 text-xs mt-1")
                if state.after_path:
                    ui.label(state.after_path).classes("text-gray-600 text-xs mt-1").style("max-width: 280px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;")
            with ui.column().classes("gap-1 items-end"):
                ui.button("🗑 移除", on_click=remove_after).props("flat dense").classes("text-gray-500 hover:text-red-400 text-xs")
                ui.button("📂 浏览...", on_click=browse_after).props("flat dense").classes("text-blue-400 text-xs")
